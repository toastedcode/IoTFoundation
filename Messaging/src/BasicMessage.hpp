...
private:
   ...
   typedef Map<String, String, MAX_PARAMETERS> ParameterMap;
...
inline char BasicMessage::getChar(ParameterName name) const
{
   ...
   return (((value = parameters.get(name)) != 0) ? value->charAt(0) : 0);
}
...
{
   const String* value = 0;
   return (((value = parameters.get(name)) != 0) ? value->toFloat() : 0);
}

inline int BasicMessage::getInt(ParameterName name) const
{
   const String* value = 0;
   return (((value = parameters.get(name)) != 0) ? value->toInt() : 0);
}

inline String BasicMessage::getString(ParameterName name) const
{
   const String* value = 0;
   return (((value = parameters.get(name)) != 0) ? *value : "");
}

inline void BasicMessage::set(ParameterName name, bool value)
{
   parameters.put(name, ((value == true) ? "true" : "false"));
};
